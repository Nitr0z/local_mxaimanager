...
class ollama_test extends \base_testcase
{
    ...
    public function test_constructor_with_valid_config(): void
    {
        ...
        $this->assertInstanceOf(\local_mxaimanager\app\ai\provider\providers\ollama::class, $provider);
    }
    ...
    public function test_constructor_missing_base_url(): void
    {
        ...
        $this->expectExceptionMessage('Ollama is missing base url and/or api key');

        new \local_mxaimanager\app\ai\provider\providers\ollama(
            $this->mock_base_factory,
            $json_config
        );
    }
    ...
    public function test_chat_completion_success(): void
    {
        ...
        $expected_response = '{"message":{"content":"Hello, world!"},"done_reason":"stop","prompt-eval-count": 1,"eval-count": 1}';
        ...
        $this->mock_curl->expects($this->once())
            ...
            ->with(
                ...
                $this->callback(function ($data) {
                    ...
                    return isset($decoded['model'], $decoded['stream'], $decoded['messages']) && $decoded['model'] === 'some-chat-model';
                })
            )
    ...
    public function test_chat_completion_finish_reason_length(): void
    {
        ...
        $expected_response = '{"message":{"content":"{\\"partial\\":"},"done_reason":"length","prompt-eval-count": 1,"eval-count": 1}';
    ...
    public function test_chat_completion_finish_reason_defaults_to_stop(): void
    {
        ...
        // Response without done_reason field
        $expected_response = '{"message":{"content":"Hello"},"prompt-eval-count": 1,"eval-count": 1}';
    ...
    public function test_get_embedding_success(): void
    {
        ...
        $expected_response = '{"embeddings":[[0.1, 0.2, 0.3]],"prompt_eval_count":1}';

        $this->mock_curl->expects($this->once())
            ->method('post')
            ->with(
                'https://api.ollama.com/api/embed',
                $this->callback(function ($data) {
                    $decoded = json_decode($data, true);
                    return isset($decoded['model'], $decoded['input']) && $decoded['model'] === 'some-embedding-model';
                })
            )
            ->willReturn($expected_response);
    ...
    public function test_get_embedding_with_dimension(): void
    {
        ...
        $this->mock_curl->expects($this->once())
            ...
            ->with(
                ...
                $this->callback(function ($data) {
                    $decoded = json_decode($data, true);
                    return isset($decoded['model'], $decoded['input'], $decoded['options']['dimensions']) && $decoded['model'] === 'some-embedding-model' && $decoded['options']['dimensions'] === 512;
                })
            )
            ->willReturn($expected_response);
    ...
    public function test_moodleform_definition(): void
    {
        ...
        \local_mxaimanager\app\ai\provider\providers\ollama::moodleform_definition(
            $mform,
            $element_name_prefix
        );
    ...
    {
        $data = [
            'prefix_base_url' => 'https://api.ollama.com',
            'prefix_api_key' => 'test_key',
            'prefix_chat_model' => 'some-chat-model',
            'prefix_embedding_model' => 'some-embedding-model'
        ];

        $errors = \local_mxaimanager\app\ai\provider\providers\ollama::moodleform_validation(
            $data,
            'prefix_'
        );
    ...
    public function test_action_moodleform_definition_chat_completion(): void
    {
        ...
        \local_mxaimanager\app\ai\provider\providers\ollama::action_moodleform_definition(
            $mform,
